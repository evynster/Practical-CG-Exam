...
void IlluminationBuffer::ApplyEffect(GBuffer* gBuffer)
{
	...
	/*
	_shaders[2]->Bind();
	_shaders[2]->SetUniformMatrix("u_LightSpaceMatrix", _lightSpaceViewProj);
...
PointLight& IlluminationBuffer::getPointRef()
{
	return _light;
}
...
void IlluminationBuffer::SetLight(PointLight newLight)
{
	_light = newLight;
}

void IlluminationBuffer::SetLight(glm::vec4 lightPos, glm::vec4 lightCol)
{
	_light._lightPosition = lightPos;
	_light._lightCol = lightCol;
}
