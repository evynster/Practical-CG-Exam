...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			if (ImGui::CollapsingHeader("COC Settings"))
			{
				if (ImGui::Checkbox("Light Mesh", &(drawLightStuff)))
				{
				}
				if (ImGui::DragFloat("zNear", &dof->zNear, 0.1f, -100.0f, 100.0f))
				{
				}
				if (ImGui::DragFloat("zFar", &dof->zFar, 0.1f, -100.0f, 100.0f))
				{
				}
				if (ImGui::DragFloat("planeInFocus", &dof->planeInFocus, 0.01f, -100.0f, 1000.0f))
				{
				}
				if (ImGui::DragFloat("focalLength", &dof->focalLength, 0.001f, -10.0f, 10.0f))
				{
				}
				if (ImGui::DragFloat("aperture", &dof->aperture, 0.001f, -10.0f, 10.0f))
				{
				}					
			}
			...
			if (ImGui::CollapsingHeader("Point Light Lighting Settings"))
			{
				if (ImGui::Checkbox("Light Mesh", &(drawLightStuff)))
				{
				}
				if (ImGui::DragFloat3("Light Position", glm::value_ptr(illuminationBuffer->getPointRef()._lightPosition), 0.01f, -10.0f, 10.0f))
				{
				}
				if (ImGui::DragFloat3("Light Color", glm::value_ptr(illuminationBuffer->getPointRef()._lightCol), 0.1f, 0.0f, 1.0f))
				{
				}
				if (ImGui::DragFloat("Light Mesh", &(illuminationBuffer->getPointRef().volume), 0.1f, 0.1f, 3.0f))
				{
				}
			}
		...
		GameObject lightOBJ = scene->CreateEntity("LightOBJ");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/bread.obj");
			lightOBJ.emplace<RendererComponent>().SetMesh(vao).SetMaterial(boxMat);
			lightOBJ.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
		} 
		...
		GameObject dofObject = scene->CreateEntity("Dof Effect");
		{
			dof = &dofObject.emplace<Dof>();
			dof->Init(width, height);
		}
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			lightOBJ.get<Transform>().SetLocalPosition(illuminationBuffer->getPointRef()._lightPosition);
			lightOBJ.get<Transform>().SetLocalScale(illuminationBuffer->getPointRef().volume, illuminationBuffer->getPointRef().volume, illuminationBuffer->getPointRef().volume);
			...
			if (drawGBuffer) {
				...
				effects[activeEffect]->ApplyEffect(illuminationBuffer);
				//effects[activeEffect]->DrawToScreen();
				//dof->ApplyEffect(illuminationBuffer);
				dof->ApplyEffect(gBuffer);
				dof->DrawToScreen();
			}
